...
* **Layer**: **In front of pony** (default) or **Behind pony**. Follow effects and planted effects that still overlap the character paint in that order relative to the body. Loose planted props still Y-sort with the herd.
...
* **Size**: Optional on-screen size relative to packed sheet pixels (**25–200%**, default **100%**). Use this when a character looks too big or small next to the herd (for example pack tiny art at **200%** for crisp pixels, then set Size to **75%**). Wallpaper **Character size** still applies on top. Written as `<scale>` (a multiplier; `0.75` for 75%) and omitted at 100%. **Check…** previews apply Size and draw a dashed stock-pony-height guide.
...
* **Left/right sprite** (Back/Front when Movement is Vertical wander or Vertical only):
    ...
    * **Import frames**: Opens a folder of PNGs and handles packing, scaling, and per-frame **lift** / **nudge**. Scale is nearest-neighbour: **200%** (×2) for undersized pixel art, **100%** native, **50%** (Desktop Ponies), down to **6.25%**, plus **Fit to built-in**. Fit is auto-selected when frames are taller than a stock pony; 200% is never auto-selected. Herd-relative size is the Pony **Size** field, not a 150% pack. Lifts and nudges are in **output** pixels (after scale). Lift is air under the sprite (0 = on the ground); nudge is pixels right of centre (negative = left). Use nudge when a tight crop ate the side padding that kept a frame registered. **Apply lift to all** / **Apply nudge to all** copy the current spinner to every frame; **Reset lifts** / **Reset nudges** clear to 0; **Apply hop** builds a lift parabola. Drag the selected-frame preview to set both. **Mirror** (Ctrl+M) flops that frame and negates its nudge. **Clone** (Ctrl+D) duplicates the selected frame after it; **Delete** removes it (the last frame cannot be removed). **Reset order** restores the import, including deleted frames and undoing mirrors. If you 2× a sheet that already has explicit anchors, re-pick or scale those anchors.
    * **Copy to right/left** (or front/back): Uses this sheet, timings, and anchors for both facings with **no flop**. For front-facing idles, symmetric chibis, and any clip that should look the same both ways. When both sides already match, the inspector collapses to **Sprites — both facings** (import/timings apply to both); **Unlink facings** shows the two panes again.
    * **Mirror to right/left** (or front/back): Builds the opposite facing by flopping cells. Distinct back art usually needs its own import — a flopped side view is not a convincing back. Do not use Mirror when you only have one drawing and it should not be flopped.
