...
### Herd drain (mix shuffle / roster reload)
...
Drain leave picks the **fastest** leave-capable next-moving slot (uniform among ties) so the default gait bag cannot roll stroll/walk for the exit, and the destination is the **nearest allowed gutter** (left/right at current Y for horizontal/`any`; top/bottom at current X for vertical) rather than a coin-flip that can send a pony across the screen. A pony already mid-walk is retargeted to that nearer gutter and upgraded to a faster NORMAL gait **on the same facing axis** (back/front vertical clips stay vertical; they are not swapped for a faster left/right trot). Incidental 1-in-8 scene exits still pick a random gutter. Tableau reloads still hold the posed scene until the incoming herd is ready.
...
- **Note**: Drag actions must always resolve to a real action. Start and/or Crossing must list at least one real action between them.
