...
Wander (and World Flow) roster changes — mix shuffle, Load mix, checkbox edits, Reload herd — do not wipe the live herd. On-screen ponies are forced to leave with existing exit movers (`screen-out`, teleport-out, walk/fly to a gutter). The inactive pool does not refill. Incoming sprite pin/decode does **not** overlap that walk-off. When the stage is empty (or after **15 s**), the old herd unloads, then the new mix is decoded and spawned on an empty screen. Reload herd syncs/bumps library generation so every host drains once; the host that wrote the bump ignores the echo so the incoming herd is not drained a second time.
