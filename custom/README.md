...
* Anchor X (`<anchorx>`): Optional. Unscaled pixel column (from the **left** of each frame) where the pony’s feet / body origin sit. Leave empty (or omit the XML element) for normal sheets that are already centred in the frame. Set this when asymmetric VFX or padding would otherwise slide the body sideways when switching actions. Example: `42`. CLI: `-anchorx 42` or `-anchorx none` to clear.
* Anchor Y (`<anchory>`): Optional. Unscaled pixel row (from the **top** of each frame) where the pony’s feet sit. Leave the field empty (or omit the XML element) for normal sheets that are already bottom-aligned. Set this on tall VFX sheets—especially teleports—so the body does not jump when switching to a shorter stand/walk sheet. Example: `57`. CLI: `-anchory 57` or `-anchory none` to clear.
...
If a sheet is also wider or padded unevenly (VFX mostly on one side), the body can **slide** sideways on action change. Set optional `<anchorx>` to the feet column (pixels from the left of a single frame); leave it unset to keep the default frame centre.
