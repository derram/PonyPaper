...
- **Saved backgrounds** — each selected image is kept in an album; the live wallpaper stays on the last pick. Long-press Select background to add several images without changing the wallpaper. The screen saver can cycle the album (default off) on a 5–30 minute interval, on top of the slow OLED pan. Swipe left or right on the saver to skip (needs two saved images; does not change the live wallpaper). **Background fit** is Fill screen (crop) or Show entire image (letterbox on the base colour). The wallpaper defaults to fill; the screen saver defaults to the whole image. Thin letterboxes (under 8% of the long edge) snap to fill. **Pixelation** is per host: the wallpaper defaults to 4 (chunky, matching the sprites); the screen saver defaults to 1 (full fitted resolution).
...
<table>
  <tr>
    ...
    <td><img src='screenshots/screensaver-clock.png' width='180'></td>
    <td><img src='screenshots/custom.png' width='180'></td>
  </tr>
  <tr>
    <td><img src='screenshots/preferences1.png' width='180'></td>
    <td><img src='screenshots/preferences4.png' width='180'></td>
...
5. (Optional) Use as a **screen saver**: system **Settings → Display → Screen saver** (wording varies by OEM) → choose **Pony Paper**. From in-app settings you can also open **Screen saver settings**. The screensaver shares the wallpaper herd by default; under **Screen saver** you can pick a different **scene mode** (and a saved Tableau scene when that mode is Tableau), optionally override pony count / FPS / background, choose **Background fit**, enable a large digital **clock** (and date), **cycle saved backgrounds**, and set how long the saver stays on with no touch (**Turn off after inactivity**). With two or more saved backgrounds, swipe left or right on the saver to skip. On Pixel that defaults to 10 minutes. On other phones it stays **Never** until you pick a duration and allow lock, which is what actually powers the panel off; you can remove that permission later from the same Screen saver section. Select a background more than once (or pick different images) to fill **Saved backgrounds**; cycling needs at least two.
